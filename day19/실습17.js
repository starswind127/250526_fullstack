...
function addWaiting() {
    ...
    let newNo = waitingList.length > 0 ? waitingList[waitingList.length - 1].no + 1 : 1;
    ...
    alert(`대기 등록 완료! 대기번호는 ${newNo}번입니다.`);
...
function checkStatus() {
    ...
    const found = waitingList.find(w => w.phone === phone);
    if (found) {
        alert(`고객님의 대기번호는 ${found.no}번입니다.`);
    } else {
        alert('대기 정보가 없습니다.');
    }
